...
- A100-40GB에서는 tier3가 2시간 안에 들어오는 가장 높은 tier다. tier2는 경계선이다(A100-80GB 3차에서는 cold 적재가 328초로 짧아 tier2 고정 환산이 6,570초로 들어왔다). L40S는 미실측이며 실행 중 측정으로 정해진다.
- dev200 F1은 규칙이 그 200건으로 튜닝됐으므로 미지 데이터 증거가 아니고, 같은 패킷의 새 추론은 동결 응답 재판정보다 약 0.015 낮았다(0.8628 vs 0.8775).
- **기본 상한은 tier2다**(`--tier-ceiling 2`, 2026-09-19 사용자 승인). tier0/1은 tier2보다 비싸면서 새 추론 F1이 두 번 모두 낮았다(canonical 0.8587·0.8628 vs tier2 0.8809). 상한 2는 tier2 고정이 아니다: 감당되지 않으면 tier3로 내려간다. `--tier-ceiling 0`으로 canonical 재현이 가능하다.
...
## 비용 모델과 tier 정책 (`CostModel`, `TierPolicy`)
...
- 개발 실행에서 1853건 마감을 흉내 내려면 `--projection-records 1853`. 남은 레코드 수만 그 값으로 계산하고 정지 규칙은 실제 마감을 쓴다.
...
- 엔진 예외가 나도 `failure.json`을 남긴 뒤 완성 CSV를 쓰고 예외를 다시 던진다. **CSV를 보류하는 경로는 없다.**
...
| `run_report.json` | 카운터, tier 분포·전환 타임라인, `cost_model`(a, b, 창 수), 프로파일별 토큰·캐시·존재율, 마감 상태 |
...
5. 아직 없는 것: 실제 1853건 완주, 비공개 입력의 길이 분포에 따른 준비·prefill 부하, L40S 실측, 공식 unseen 점수.
