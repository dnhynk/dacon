...
2026-09-12 기준 과거 저장 응답 재판정의 최고 개발 기록은 **0.751807**입니다.
같은 입력·후처리 경로로 답변을 새로 생성한 개발 160건에서는 **0.706466**을 기록해
최고점을 재현하지 못했습니다. 두 기록과 원본을 모두 보존합니다.
**공식 리더보드 점수가 아닙니다.** 0.718474도 이전 비교 기준으로 보존합니다.
...
| 동일 A/L 입력·v1 CPU, 새 전체 실행 | 0.706466 | 60 / 28 | 개발 160건, 새 응답 640개, 재시도 0회 |
| 위 새 응답의 조건부 v20 재검토 | 0.710923 | 55 / 28 | 같은 새 응답 중 498개를 사용하는 별도 CPU 경로 비교 |

과거 최고 기록은 **저장 응답을 재사용한 개발 결과**이며, 새 추론에서 보장되는 성능이
아닙니다. 이번 새 실행에서는 과거 대비 오류 10개 회복·21개 회귀가 발생했습니다.
입력과 CPU 재판정은 독립 대조가 일치했지만 모델 응답 변동의 원인은 확정하지 않았습니다.
새 공고에서의 일반화와 L40S 전체 실행 시간은 미검증입니다.

새 실행의 기본 3회 경로와 4회 경로는 총점이 같았으나 v20의 개별 판정은 달랐습니다.
추가 호출은 FP 5개를 고치면서 다른 FP 5개를 만들었습니다. 기본 v20 양성에만
추가 응답을 사용하는 사전 고정 CPU 후보는 TP 4개를 유지하고 FP를 5개 줄였습니다.
이는 실제 응답 부분집합 비교이며 선택 호출의 실행시간이나 독립 검증 결과가 아닙니다.
SW 구성품의 적용 범위가 미확정인 사례도 남아 있어 자동 채택하지 않았습니다.
...
최신 640응답 A100 실행은 모델 적재 324.1초, 적재·추론 합계 1,489.0초였습니다.
환경 준비·다운로드와 사용자의 응답을 기다린 시간은 제외한 수치입니다.
공고 수에 비례하는 단순 외삽으로도 1,853건은 2시간을 넘으며, L40S 실측은 아닙니다.
...
원래 A/L 입력 생성기와 CPU 소비기를 연결한 별도
[일반 입력 진입점](experiments/precision_joined_v1/b4_entry_candidate_20260912/source/script.py)은
현재 입력에서 패킷을 만들고, 이번 640응답을 주입했을 때 비트·근거·CSV를 재현했습니다.
이 연결부 자체의 GPU 실행과 제출 적합성은 미검증이며 필수 근거 누락 2건도 남아 있습니다.
