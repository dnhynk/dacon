...
def supports_sw_quote(rec, source):
    if not source:
        return False
    quote = source['text']
    if re.search(r'교육(?:용|과정|교재)|실습|연구\s*수행.{0,25}(?:도구|프로그램)|투자유치|일반\s*PC', quote):
        return False
    if rec is not None:
        observed = sw_check(rec)['facts']['actual_work']
        if any(e['evidence']['doc_index'] == source['doc_index']
               and e['evidence']['start'] < source['end'] and source['start'] < e['evidence']['end'] for e in observed):
            return True
    software = re.search(r'소프트웨어|정보시스템|웹\s*(?:사이트|서비스)|모바일\s*앱|라이선스|S/W|\bSW\b', quote, re.I)
    obligation = re.search(r'개발|구축|유지\s*(?:보수|관리)|라이선스\s*(?:구매|갱신)|납품|인도', quote)
    return bool(software and obligation and not unresolved_assertion(quote)
                and not re.search(r'수행을\s*위한|계약상대자.{0,30}비용|활용하여|이용하여|활용\s*도구|타당성|방안\s*연구', quote))
