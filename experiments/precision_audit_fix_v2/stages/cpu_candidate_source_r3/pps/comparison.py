...
def _scope_context(text, lo, hi):
    ...
    if lo:
        ...
        separate = bool(re.match(r'\s*(?:\d+[.)]|[가-하][.)]|[①-⑳])', text[lo:hi]))
        governing_example = bool(re.fullmatch(r'\s*(?:※\s*)?(?:예시|작성\s*예|가정)\s*[:：]?\s*', previous))
        if (not separate or governing_example) and (_CONDITIONAL.search(previous) or _NEGATED.search(previous)):
            lo = prev_start
        ...
        for match, area, lo, hi, header, table in _amount_parts(text):
            ...
            for money in _WON.finditer(area):
                ...
                if value is not None:
                    values.append(value)
                    tail = area[money.end():]
                    # "추정가격 5억원 미만" is an interval boundary, not
                    # an assignment of this project's price to 500 million.
                    if re.match(r'\s*(?:미만|이하|초과|이상|부터|까지|[~∼～])', tail):
                        bounded_value = True
            ...
            context = header + ' ' + area
            scope = ('threshold' if bounded_value else
                     'partial' if _PARTIAL.search(context) else
