...
SYSTEM_V6 = SYSTEM_V5 + """
[출처와 판단 보류]
같은 필드의 본문·메타 금액이 다르면 어느 한쪽으로 숫자를 몰래 바꾸지 않는다.
모든 관측값이 같은 법정 구간에 속하면 그 구간만 확정할 수 있으며, 정확한 금액의 충돌은 남는다.
...
RUBRIC_V6 = {**RUBRIC_V5,
    ...
    9: RUBRIC_V5[9] + " Chipset 등의 필드에 적힌 고유 제품명도 신규 납품의 필수조건이면 검토한다. 반대로 장비 종류명(C-ARM 등)만으로 특정 모델을 추측하지 않는다. 규격서 참조만 있고 그 규격서가 없으면 그 안의 제조사나 모델을 만들어내지 않는다.",
    20: "[위반] 먼저 계약에서 인도·개발·운영해야 하는 실제 SW 산출물과 그 원문 S번호를 특정한다. AI·디지털이라는 사업명, 교육과정에서 쓰는 프로그램, 연구수행 도구, 일반 PC의 내장 SW, 투자유치 프로그램은 그 표현만으로 SW사업이 아니다. 구체적인 SW 개발·구축·유지관리·라이선스 갱신 의무가 있는지 확인한다. SW 포함 사업이면 해당 부분과 전체 사업을 구별한다. 실제 SW사업임이 확인된 뒤 하한제도 적용 여부와 적용 근거가 누락됐는지 판정한다. 제48조와 사업금액별 참여제한의 실제 적용 문구가 있으면 안내는 존재한다. '적용되는지 불명확', '안내 생략', '예시'는 적용 안내가 아니다. SW사업자 등록·중소기업 확인서·상호출자제한기업 금지만으로 하한제도 안내를 대체하지 않는다. SW사업 자체를 확인할 수 없으면 사실 요약에 미확정과 빠진 정보를 쓰고, 도구를 납품 SW로 간주하지 않는다.",
