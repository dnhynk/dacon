...
from .fact_contract import STATES as TYPED_FIELDS, schema as fact_schema, instructions as fact_instructions
...
class Config:
    ...
    allow_legacy_model_fact_join: bool = False

    def __post_init__(self):
        if type(self.allow_legacy_model_fact_join) is not bool:
            raise ValueError('allow_legacy_model_fact_join must be boolean')
        if self.response_format == 'typed_fact_compact' and self.allow_legacy_model_fact_join:
            raise ValueError('Typed facts cannot enable the legacy prose join')
...
def fact_fields(items, response_format=None):
    ...
    if response_format == 'typed_fact_compact' and 9 in items:
        from .fact_contract import MODEL
        fields.append(MODEL)
    return fields
...
def output_schema(response_format="compact", max_evidence=None, items=tuple(range(1, 25))):
    ...
    if response_format in {"reasoned", "factored", "fact_compact", "typed_fact_compact"}:
        ...
        if response_format == "reasoned":
            return judgments
        if response_format in {"fact_compact", "typed_fact_compact"}:
            judgments = output_schema("compact", max_evidence, items)
        names = fact_fields(items, response_format)
        ...
        if response_format == 'typed_fact_compact':
            facts['properties'].update({key: fact_schema(key, max_evidence) for key in names if key in TYPED_FIELDS})
...
def build_prompt(rec, knowledge, config, tokenizer=None, items=tuple(range(1, 25))):
    ...
    if config.response_format in {"fact_compact", "typed_fact_compact"}:
...
def build_shared_prompts(rec, knowledge, config, tokenizer, groups):
    ...
    """
    if config.rubric_version not in {"v3", "v4", "v5", "v6"} or config.response_format not in {"reasoned", "factored", "fact_compact", "typed_fact_compact", "compact"}:
...
"""
    ...
    from .source_roles import observations, ranges as role_ranges, packet as role_packet
    observed_roles = observations(rec, all_items) if config.response_format == 'typed_fact_compact' else []
    ...
    for items in groups:
        ...
        elif config.response_format in {"fact_compact", "typed_fact_compact"}:
            suffix += ("최종 JSON은 {\"facts\":{사실항목:사실값},\"judgments\":{\"v\":[0또는1,...],\"e\":[원문구간번호,...]}}이다. "
                       "facts를 먼저 작성하고 그 사실에 법령조건을 적용해 judgments를 쓴다. "
                       + ("객체로 지정되지 않은 사실값은 220자 이내 설명이며 확인할 수 없으면 불명확하다고 쓴다. "
                          if config.response_format == 'typed_fact_compact' else
                          "각 사실값은 220자 이내 설명이며 확인할 수 없으면 불명확하다고 쓴다. ") +
                       "원문 자격조건은 S번호와 함께 요약한다. 합법적 요건의 존재를 위반으로 뒤집지 않는다. "
                       "facts 필수키: " + ", ".join(fact_fields(items, config.response_format)) + ".\n"
        ...
        if config.response_format == 'typed_fact_compact':
            suffix += fact_instructions(fact_fields(items, config.response_format))
    ...
    while True:
        spans = index.select(budget, items=all_items, mode=config.mode,
                             priority_ranges=(priority_ranges(comparison) if comparison is not None else []) + role_ranges(observed_roles))
        coverage = index.coverage(spans)
        data = {"meta": rec["meta"], "input_completeness": rec.get("input_completeness", {}),
        ...
        for items,suffix,legal_diagnostics in zip(groups,suffixes,group_legal_diagnostics):
            ...
            role_text = ('\n\n[원문 역할과 적용 범위 검색 보조정보]\n' + json.dumps(
                role_packet(observed_roles, spans, rec, items), ensure_ascii=False, separators=(',', ':'))
                if observed_roles and set(items).intersection({7,9,20}) else '')
            messages = [{"role":"system","content":system}, {"role":"user","content":common+comparison_text+role_text+suffix}]
            ids = token_ids(tokenizer,messages,config.enable_thinking) if tokenizer is not None else None
