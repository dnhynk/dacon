...
def industry_fields(rec, *, doc_types=('공고문',)):
    ...
    for di,d in enumerate(rec['docs']):
        ...
        for a in pat.finditer(t):
            ...
            lo,hi=clause(t,a.start(),a.end());context=compact(t[lo:hi])
            if not re.search(r'등록|신고|허가',context):continue
            if not re.search(r'업체|자이어야|한자|된자|갖춘자|등록한',context):continue
            if re.search(r'경우에한|해당시|변경등록|입찰대리인',context):continue
            out.append(fact(di,t,lo,hi,'mandatory_industry_code',a[1],alternative=bool(re.search(r'또는|중하나|이거나',context))))
    return out
