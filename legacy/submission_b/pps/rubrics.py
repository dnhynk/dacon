...
# Opt-in group suffix supplements; the shared system and source stay unchanged.
FACT_JUDGMENT_CONSISTENCY = (
    "[사실–판정 일관성] facts에 S번호로 적은 해당 항목의 위반유형 제한·요건은 "
    "명시적 비위반 사유(예외·금액대 비적용 등)도 적지 않는 한 v=1이다."
)

FACT_JUDGMENT_RUBRIC = {
    1: "[위반] 특정 기관 유형·설립 형태(대학·연구기관·공공기관·산학협력단 등)만 참가시키거나 과도한 보유 시설·상시 인력 규모·전국 조직 조건으로 자격을 한정. 법정 면허·업종, 일반 업체에 비영리법인을 추가 허용, 평가 배점·우대는 제외. 과업에 비례하는 필요조건과 과도한 제한, 시설 이용 능력과 입찰 시 직접 보유 의무를 구별한다. 연구 과업만으로 대학·국공립 연구기관 한정을 정당화하지 않는다.",
    2: "[위반] 추정가격이 적용 고시금액(통상 2.3억원) 미만인 제조·용역에서 실적 보유를 입찰참가 필수조건으로 요구. 실적 금액 기재 여부와 무관하며 횟수·운영 경험도 실적이다. 계약금액이 작다는 것만으로 허용되지 않는다. 평가 배점만인 경우는 0이며 지방 소액수의의 명시적 예외를 구별한다.",
    9: "[위반] 규격·과업 문서의 신규 구매품에 특정 제조사·모델·상표·칩셋을 지정하면 같은 품목에 동등 이상 대체 허용이 명시되지 않는 한 대상이다. 기존 장비 설명·유지보수 대상, 동등 이상을 명확히 허용한 예시는 0. 액세서리 수량 허용을 본체 대체 허용으로 넓히지 않는다. 포괄적 동등규격 제목 아래 남은 필수 제품 지정도 확인한다. 숫자·영문 성능규격과 고유 제품명을 구별하고 Chipset 필드의 고유 제품명도 검토한다. 장비 종류명(C-ARM 등)이나 미제공 규격서 참조만으로 제조사·모델을 추측하지 않는다.",
    19: "[위반] 제3자 제조사·공급사의 공급·기술지원 확약서(동일 기능 증명서)를 입찰서 제출 마감 전/입찰 시 발급·보유·제출할 의무. 입찰 전 발급 후 계약시 제출도 1. 낙찰 후 확보·계약시 제출, 발주처 사전협약, 입찰자 이행서약, 단순 제조사 사실확인·대리점 인증은 0. 계약 전/납품 전을 입찰 전으로 읽지 않는다. 제출 가능 능력만이면 제외.",
}
