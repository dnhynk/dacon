...
ACTION = re.compile(r'제출|보유|소지|확보|발급\s*받|발급')
EARLY = re.compile(r'(?:전자\s*)?입찰(?:서)?\s*(?:제출\s*)?마감일?\s*전|입찰\s*(?:전(?:일|까지|에)?|시)|낙찰통보\s*(?:이전|전)')
LATE = re.compile(r'낙찰(?:\s*통보|자\s*결정|자로\s*결정된)?\s*(?:후|이후)|계약\s*(?:체결\s*)?(?:시|전|후|때)|착수\s*전|납품\s*(?:전|후)')
...
OTHER_DOCUMENT = re.compile(r'(?!확약서|협약서)[가-힣A-Za-z]+(?:서|증|서류)(?:은|는|이|가|을|를|만)')
...
def analyze(text):
    ...
    for match in SENTENCE.finditer(text):
        ...
        for i, action in enumerate(actions):
            # A preceding self-written undertaking has its own submission.
            # Its date must not become the third-party pledge's deadline.
            if action.start() < target.start() and OTHER_DOCUMENT.search(unit[:action.start()]):
                continue
            ...
            negated = bool(re.match(r'(?:할|받을|을)?(?:필요|의무)(?:가|는|도)?없|'
                ...
                r'(?:은|는|이)?(?:면제|불요)|하지(?:않|아니)|하지않아도|'
                r'여부(?:는|를|도)?(?:입찰|참가)?자격(?:으로|에서|에)?'
                r'(?:심사|평가|반영)하지(?:않|아니)', tail))
            ...
            action_name = ('submit' if action.group() == '제출' else
                           'hold' if action.group() in {'보유', '소지', '확보'} else 'issue_or_receive')
...
def apply(pledge):
    ...
    elif (facts['events'] and all(e['stage'] == 'explicit_later_stage'
            and e['modality'] in {'required', 'capability', 'negated'} for e in facts['events'])):
        pledge['timing'] = 'explicit_later_stage'
    return facts
