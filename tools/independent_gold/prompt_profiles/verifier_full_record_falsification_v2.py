"""Blind ID-only falsification prompt for one-call 24-item verification."""
...
PROFILE_NAME = "verifier-full-record-falsification-v2"
ANNOTATOR_ROLE = "verifier"
REQUIRED_MODEL_FAMILY = "openai:gpt-6"
PROMPT_PROTOCOL_VERSION = (
    "dacon.independent.verifier_full_record_falsification_prompt.v2"
)
...
6. `allowed_span_registry` is the complete gapless organizer source registry.
   Emit only exact registry IDs in `source_span_ids`, once per selected span,
   and cite selected IDs from each decision. Do not copy quote, doc_index,
   start, or end into the answer. The validator reconstructs the source span
   from organizer text and rejects unknown, forged, duplicated, or unused IDs.
