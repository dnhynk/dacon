"""ID-only, source-bound prompt for one-call 24-item candidate annotation."""
...
PROFILE_NAME = "candidate-full-record-source-direct-v2"
ANNOTATOR_ROLE = "candidate"
REQUIRED_MODEL_FAMILY = "openai:gpt-5.6"
PROMPT_PROTOCOL_VERSION = (
    "dacon.independent.candidate_full_record_source_direct_prompt.v2"
)
...
5. ID-ONLY SOURCE CONTRACT: `allowed_span_registry` is a gapless rendering of
   all supplied documents. Emit only its exact span IDs in `source_span_ids`.
   Each decision cites those selected IDs in `premise_span_ids`; cite a selected
   premise ID in `positive_evidence_span_id` when required. Select each used
   ID once, with no unused IDs. Never copy source quote, doc_index, start, or
   end into the answer. The validator reconstructs them from the organizer
   source; it rejects unknown, forged, duplicated, or unselected IDs.
